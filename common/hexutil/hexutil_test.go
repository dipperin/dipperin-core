...
func TestMustDecodeBig(t *testing.T) {
	x := MustDecodeBig("0x123")
	assert.Equal(t, big.NewInt(0x123), x)
	...
	assert.NotEmpty(t, ReferenceBytes("123a"))
	assert.Panics(t, func() {
		ReferenceBytes("0xqwe")
	})

	ux := MustDecodeUint64("0x12")
	assert.Equal(t, uint64(0x12), ux)
	assert.Panics(t, func() {
		MustDecodeUint64("0xqwe")
	})

	assert.NotEmpty(t, MustDecode("0x12"))
	assert.Panics(t, func() {
		MustDecode("0xqwe")
	})

	EncodeWithout0x([]byte("123"))
}