...
func TestMore(t *testing.T) {
	x := limitSigned256(NewUint256(big.NewInt(0).Add(tt255, tt255)))
	x.Bytes()
	assert.Equal(t, "0", x.String())
	assert.Equal(t, Int256(22).String(), Big(22).Int256().String())
	assert.Equal(t, 1, Big(22).FirstBitSet())
	assert.Equal(t, "22", Big(22).String())
	assert.Equal(t, "6", Big(0).Mul(Big(2), Big(3)).String())
	assert.Equal(t, "6", Big(0).Div(Big(12), Big(2)).String())
	assert.Equal(t, "6", Big(0).Mod(Big(20), Big(7)).String())
	assert.Equal(t, "80", Big(0).Lsh(Big(20), 2).String())
	assert.Equal(t, "400", Big(0).Pow(Big(20), Big(2)).String())
}
