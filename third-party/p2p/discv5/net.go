...
func (net *Network) SetFallbackNodes(nodes []*Node) error {
	...
	for _, n := range nodes {
		...
		cpy := *n
		cpy.sha = cs_crypto.Keccak256Hash(n.ID[:])
...
func (net *Network) Resolve(targetID NodeID) *Node {
	result := net.lookup(cs_crypto.Keccak256Hash(targetID[:]), true)
...
func (net *Network) Lookup(targetID NodeID) []*Node {
	return net.lookup(cs_crypto.Keccak256Hash(targetID[:]), false)
}
	...
	for {
		...
		case pkt := <-net.read:
			...
			if err := net.handle(n, pkt.ev, &pkt); err != nil {
				log.DLogger.Error("read: handle processes packets sent", zap.Error(err), zap.Any("node", n))
			}
		...
		case timeout := <-net.timeout:
			...
			if err := net.handle(timeout.node, timeout.ev, nil); err != nil {
				log.DLogger.Error("timeout: handle processes packets sent", zap.Error(err), zap.Any("node", timeout.node))
			}
...
func (net *Network) handleQueryEvent(n *Node, ev nodeEvent, pkt *ingressPacket) (*nodeState, error) {
	switch ev {
	case findnodePacket:
		target := cs_crypto.Keccak256Hash(pkt.data.(*findnode).Target[:])
