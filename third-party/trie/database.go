...
type cachedNode struct {
	...
	flushNext common.Hash // NextMiss node in the flush-list
}
...
func (db *Database) Cap(limit common.StorageSize) error {
	...
	if flushPreimages {
		for hash, preimage := range db.preimages {
			// only for debug
			//hkb := hash.Bytes()
			//shouldBe := crypto.Keccak256(preimage)
			//if !bytes.Equal(hkb, shouldBe) {
			//	panic(fmt.Sprintf("Cap commit but key not match. hkb: %v should: %v originK: %v", hkb, shouldBe, string(preimage)))
			//}
...
func (db *Database) Commit(node common.Hash, report bool) error {
	...
	for hash, preimage := range db.preimages {
		...
		//shouldBe := crypto.Keccak256(preimage)
		//if !bytes.Equal(hkb, shouldBe) {
		//	panic(fmt.Sprintf("Commit but key not match. hkb: %v should: %v originK: %v", hkb, shouldBe, string(preimage)))
		//}
...
func (db *Database) verifyIntegrity() {
	...
	for hash, node := range db.nodes {
		if _, ok := reachable[hash]; !ok {
			unreachable = append(unreachable, fmt.Sprintf("%x: {Node: %v, Parents: %d, Prev: %x, NextMiss: %x}",
