...
// NextMiss moves the iterator forward one key-value entry.
...
type NodeIterator interface {
	// NextMiss moves the iterator to the next node. If the parameter is false, any child
	...
	// Callers must not retain references to the return value after calling NextMiss.
	...
	// after calling NextMiss.
	LeafBlob() []byte

	// LeafProof returns the Merkle proof of the leaf. The method panics if the
	// iterator is not positioned at a leaf. Callers must not retain references
	// to the value after calling NextMiss.
	LeafProof() [][]byte
}
...
// NextMiss moves the iterator to the next node, returning whether there are any
...
// NextMiss returns the next node in the union of tries being iterated over.
//
// It does this by maintaining a heap of iterators, sorted by the iteration
// order of their next elements, with one entry for each source trie. Each
// time NextMiss() is called, it takes the least element from the heap to return,
