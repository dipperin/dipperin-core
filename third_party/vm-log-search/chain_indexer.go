...
type ChainIndexerChain interface {
	...
	SubscribeChainHeadEvent(ch chan<- ChainHeadEvent) gevent.Subscription
}
...
func (c *ChainIndexer) ServiceFilter(ctx context.Context, session *MatcherSession) {
	for i := 0; i < bloomFilterThreads; i++ {
		go session.Multiplex(bloomRetrievalBatch, bloomRetrievalWait, c.BloomRequests)
	}
}
...
// queue.
func (c *ChainIndexer) eventLoop(currentHeader *model.AbstractHeader, blockChan chan model.Block, sub gevent.Subscription) {
	...
	c.newHead((*currentHeader).GetNumber())
...
// newHead notifies the indexer about new chain heads
