...
// the first one will be NOOP. As such, we'll use b.N as the number of accounts to
// insert into the trie before measuring the hashing.
func BenchmarkHash(b *testing.B) {
	// Make the random benchmark deterministic
	random := rand.New(rand.NewSource(0))

	// Create a realistic accounts trie to hash
...
func TestNew(t *testing.T) {
	...
	updateString(trie, "dog", "puppy")
	updateString(trie, "dogglesworth", "cat")
	updateString(trie, "doggleswor", "bbb")
	testroot1, _ := trie.Commit(nil)
	fmt.Print("size1: ", testroot1)
	fmt.Println(triedb.Size())

	deleteString(trie, "doe")
	testroot2, _ := trie.Commit(nil)
	fmt.Print("size2: ")
	fmt.Println(triedb.Size())

	deleteString(trie, "dogglesworth")
	testroot3, _ := trie.Commit(nil)
	fmt.Print("size3: ")
	fmt.Println(triedb.Size())

	//可以改下面的testroot1,testroot2,testroot3看效果
	triedb.Dereference(testroot2)
	fmt.Print("size derefed: ")
	fmt.Println(triedb.Size())

	tire1, err1 := New(testroot1, triedb)
	if err1 == nil {
		fmt.Println("1", string(tire1.Get([]byte("doe"))))
		fmt.Println("2", string(tire1.Get([]byte("dog"))))
		fmt.Println("3", string(tire1.Get([]byte("doggleswor"))))
		fmt.Println("4", string(tire1.Get([]byte("dogglesworth"))))
	} else {
		fmt.Println(err1)
	}

	tire2, err2 := New(testroot2, triedb)
	if err2 == nil {
		fmt.Println("1", string(tire2.Get([]byte("doe"))))
		fmt.Println("2", string(tire2.Get([]byte("dog"))))
		fmt.Println("3", string(tire2.Get([]byte("doggleswor"))))
		fmt.Println("4", string(tire2.Get([]byte("dogglesworth"))))
	} else {
		fmt.Println(err2)
	}

	tire3, err3 := New(testroot3, triedb)
	if err3 == nil {
		fmt.Println("1", string(tire3.Get([]byte("doe"))))
		fmt.Println("2", string(tire3.Get([]byte("dog"))))
		fmt.Println("3", string(tire3.Get([]byte("doggleswor"))))
		fmt.Println("4", string(tire3.Get([]byte("dogglesworth"))))
	} else {
		fmt.Println(err3)
	}
