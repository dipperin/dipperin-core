...
type CsPeerInfo struct {
	...
	NodeType        uint64 `json:"node_type"`
...
type CsPmPeerInfo struct {
	SelfType     uint64        `json:"self_type"`
	SelfNodeName string        `json:"self_node_name"`
	SelfNodeID   string        `json:"self_node_id"`
	Base         []*CsPeerInfo `json:"base"`
	CurVerifier  []*CsPeerInfo `json:"cur_verifier"`
	NextVerifier []*CsPeerInfo `json:"next_verifier"`
	VerifierBoot []*CsPeerInfo `json:"verifier_boot"`
}

func (c *CsPeerInfo) String() string {
	return fmt.Sprintf(`
[
	ID:   				%s
	NodeName:   		%s
...
`, c.ID, c.NodeName, c.NodeType, c.RawUrl, c.HeadHash, c.HeadHeight, c.VerifierAddress)
}

func (c *CsPmPeerInfo) String() string {
	return fmt.Sprintf(`
	Debug Info:
...
`, c.SelfType, c.SelfNodeName, c.SelfNodeID, c.Base, c.CurVerifier, c.NextVerifier, c.VerifierBoot)
}
