...
func (netType NetType) String() string {
	switch netType {
	case local:
		return chainconfig.BootEnvLocal
	case venus:
		return chainconfig.BootEnvVenus
	case test:
		return chainconfig.BootEnvTest
	case mercury:
		return chainconfig.BootEnvMercury
	default:
		return "errorNetType"
	}
}
...
func (req *findnode) handle(t *udp, from *net.UDPAddr, fromKey encPubkey, mac []byte) error {
	...
	target := enode.ID(cs_crypto.Keccak256Hash(req.Target[:]))
	t.tab.mutex.Lock()
