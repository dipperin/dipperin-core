...
func newSystemFuncSet(r *Resolver) map[string]map[string]*exec.FunctionImport {
	return map[string]map[string]*exec.FunctionImport{
		"env": {
			...
			"getProofHeight":         &exec.FunctionImport{Execute: r.envGetProofHeight, GasCost: constGasFunc(GasQuickStep)},
			"validateSPVProof":       &exec.FunctionImport{Execute: r.envValidateSPVProof, GasCost: constGasFunc(GasQuickStep)},
