...
func NewWASMInterpreter(state base.StateDB, context base.Context, vmConfig exec.VMConfig) *WASMInterpreter {
...
func (in *WASMInterpreter) Run(vm *VM, contract *Contract, create bool) (ret []byte, err error) {
	...
	if create {
		...
		if err != nil {
			if err == gerror.ErrInsufficientParams { // transfer to contract address.
				return nil, nil
			}
	...
	if !ok {
		return nil, gerror.ErrFuncNameNotFound
	}
...
func ParseInitFunctionByABI(vm *exec.VirtualMachine, input []byte, abi []byte) (params []int64, returnType string, err error) {
	...
	if _, ok := rlpList.([]interface{}); !ok {
		err = gerror.ErrInvalidRlpFormat
		return
	}
	...
	if returnType != "void" {
		err = gerror.ErrInvalidReturnType
		return
	}
	return
}
...
func findParams(vm *exec.VirtualMachine, abi []byte, funcName string, inputList []interface{}) (params []int64, returnType string, err error) {
	...
	if len(abiParam) != len(inputList) {
		log.DLogger.Error("findParams failed", zap.Error(gerror.ErrInputAbiNotMatch), zap.Int("abiLen", len(abiParam)), zap.Int("inputLen", len(inputList)))
		err = gerror.ErrInputAbiNotMatch
		return
	}
