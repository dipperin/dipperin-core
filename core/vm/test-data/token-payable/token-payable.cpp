...
EXPORT void TestToken::withdraw(){
    Address2 originAddr = dipc::origin2();
    Address2 callerAddr = dipc::caller2();
    Address2 contractAddr = dipc::address2();
    DipcAssert(originAddr == callerAddr);
    
    u256 originBalance = dipc::balance(contractAddr);
    DipcAssert(originBalance > 0);
    DipcAssert(dipc::callTransfer2(callerAddr,  originBalance) == 0);
}
