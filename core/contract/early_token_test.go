...
func TestMap(t *testing.T) {
	...
	testMap["a"] = 1
	testMap["a"] = 2
	log.DLogger.Info("the testMap is:", zap.Any("testMap", testMap))
}
...
func TestEarlyTokenUnmarshalJSON(t *testing.T) {
	...
	log.DLogger.Info("the Token owner is:", zap.String("owner", contract.Owner.Hex()))
	value, ok := contract.Balances[contract.Owner.Hex()]
	assert.EqualValues(t, true, ok)
	log.DLogger.Info("the value is:", zap.Any("value", value))
}
...
func TestUnmarshalData(t *testing.T) {
	...
	log.DLogger.Info("the bytes string is:", zap.String("bytes", string(bytes)))
	var contract EarlyRewardContract
...
func TestMakeEarlyRewardContract(t *testing.T) {
	...
	log.DLogger.Info("the remainder DIP is:", zap.Any("remainder", big.NewInt(0).Sub(economy_model.EarlyTokenDIP, testContract.NeedDIP)))
	log.DLogger.Info("the initial exchangeRate is:", zap.Int64("initialExchangeRate", economy_model.InitExchangeRate))
