...
func TestChainDB_Receipts(t *testing.T) {
	db := newChainDB()
	receipt1 := model.NewReceipt([]byte{}, false, model.TestGasLimit, nil)
	receipt2 := model.NewReceipt([]byte{}, false, model.TestGasLimit*3, nil)
...
func TestDeriveFields(t *testing.T) {
	log := &model.Log{Address: AliceAddrV}
	receipt1 := model.NewReceipt([]byte{}, false, model.TestGasLimit, []*model.Log{log})
	receipt2 := model.NewReceipt([]byte{}, false, model.TestGasLimit*3, []*model.Log{log})
	receipts := []*model.Receipt{receipt1, receipt2}
	block := createBlock(1)
	...
	expectAddr := cs_crypto.CreateContractAddress(AliceAddrV, 0)
	...
	assert.Equal(t, model.TestGasLimit, receipts[0].GasUsed)
	...
	assert.Equal(t, BobAddrV, receipts[1].ContractAddress)
	assert.Equal(t, model.TestGasLimit*2, receipts[1].GasUsed)
