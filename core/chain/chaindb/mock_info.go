...
func createSignedTx(nonce uint64, amount *big.Int, to common.Address) *model.Transaction {
	...
	testTx1 := model.NewTransaction(nonce, to, amount, model.TestGasPrice, model.TestGasLimit, []byte{})
	signedTx, _ := testTx1.SignTx(key1, fs1)
	return signedTx
}

func createBlock(num uint64) *model.Block {
	header := model.NewHeader(1, num, common.Hash{}, common.HexToHash("123456"), common.HexToDiff("1fffffff"), big.NewInt(time.Now().UnixNano()), AliceAddrV, common.BlockNonce{})

	// tx list
	to := common.HexToAddress(common.AddressContractCreate)
	tx1 := createSignedTx(0, model.TestValue, to)
	tx2 := createSignedTx(0, model.TestValue, BobAddrV)
