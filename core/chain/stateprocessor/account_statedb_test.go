...
func TestAccountStateDB_ProcessTxNew(t *testing.T) {
	...
	processor.NewAccountState(aliceAddr)
	processor.NewAccountState(bobAddr)
	processor.AddBalance(aliceAddr, big.NewInt(0).Mul(economymodel.MiniPledgeValue, big.NewInt(2)))
	...
	stackTx := fakeTransactionOpt{
		...
		nonce:  &nonce,
		sender: aliceAddr,
		amount: economymodel.MiniPledgeValue,
	}
	...
	assert.Equal(t, gerror.ErrTxNotSupported, err)
...
func TestAccountStateDB_processBasicTx_Error(t *testing.T) {
	...
	assert.Equal(t, gerror.ErrSenderOrReceiverIsEmpty, err)

	tx = fakeTransaction{sender: aliceAddr}
	conf.Tx = &tx
	err = processor.processBasicTx(conf)
	assert.Equal(t, gerror.ErrSenderNotExist, err)
	...
	conf.Tx = &tx
	err = processor.processBasicTx(conf)
	assert.Equal(t, gerror.ErrTxNonceNotMatch, err)

	tx = fakeTransaction{sender: aliceAddr}
	conf.Tx = &tx
	err = processor.processBasicTx(conf)
	assert.Equal(t, gerror.ErrBalanceNegative, err)
}
...
func TestAccountStateDB_SetError(t *testing.T) {
	...
	assert.Equal(t, gerror.ErrAccountNotExist, err)
	assert.Equal(t, gerror.ErrAccountNotExist, processor.AddBalance(aliceAddr, big.NewInt(1000)))
	assert.Equal(t, gerror.ErrAccountNotExist, processor.AddStake(aliceAddr, big.NewInt(1000)))
	assert.Equal(t, gerror.ErrAccountNotExist, processor.SubStake(aliceAddr, big.NewInt(1000)))
	assert.Equal(t, gerror.ErrAccountNotExist, processor.AddNonce(aliceAddr, uint64(5)))

	assert.Equal(t, gerror.ErrAccountNotExist, processor.SetStake(aliceAddr, big.NewInt(1000)))
	assert.Equal(t, gerror.ErrAccountNotExist, processor.SetBalance(aliceAddr, big.NewInt(1000)))
	assert.Equal(t, gerror.ErrAccountNotExist, processor.SetNonce(aliceAddr, uint64(5)))
	assert.Equal(t, gerror.ErrAccountNotExist, processor.SetLastElect(aliceAddr, uint64(5)))
	assert.Equal(t, gerror.ErrAccountNotExist, processor.SetCommitNum(aliceAddr, uint64(5)))
	assert.Equal(t, gerror.ErrAccountNotExist, processor.SetPerformance(aliceAddr, uint64(5)))
	assert.Equal(t, gerror.ErrAccountNotExist, processor.SetVerifyNum(bobAddr, uint64(5)))
	assert.Equal(t, gerror.ErrAccountNotExist, processor.SetHashLock(aliceAddr, common.HexToHash("123")))
	assert.Equal(t, gerror.ErrAccountNotExist, processor.SetDataRoot(aliceAddr, common.HexToHash("123")))
	//processor.setContractRoot(aliceAddr, common.HexToHash("123"))
	assert.Equal(t, gerror.ErrAccountNotExist, processor.SetTimeLock(aliceAddr, big.NewInt(10)))
...
func TestAccountStateDB_SetTrieError(t *testing.T) {
	processor, err := NewAccountStateDB(common.Hash{}, fakeStateStorage{setErr: TrieError})
	assert.NoError(t, err)
