...
func createUnNormalTx() *model.Transaction {
	...
	tx := model.NewUnNormalTransaction(0, big.NewInt(1000), big.NewInt(1), model.TxGas)
...
func createGenesis() *Genesis {
	...
	stateProcessor, _ := stateprocessor.MakeGenesisAccountStateProcessor(storage)
...
func (dbChain fakeAccountDBChain) StateAtByBlockNumber(num uint64) (*stateprocessor.AccountStateDB, error) {
	if num == 10 {
		db := ethdb.NewMemDatabase()
		storage := stateprocessor.NewStateStorageWithCache(db)
		return stateprocessor.NewAccountStateDB(common.Hash{}, storage)
	}
...
func (storage fakeStateStorage) OpenTrie(root common.Hash) (stateprocessor.StateTrie, error) {
	return fakeTrie{
...
func (storage fakeStateStorage) OpenStorageTrie(addrHash, root common.Hash) (stateprocessor.StateTrie, error) {
	panic("implement me")
}

func (storage fakeStateStorage) CopyTrie(stateprocessor.StateTrie) stateprocessor.StateTrie {
	panic("implement me")
}
...
func (model fakeEconomyModel) GetVerifierDIPReward(block model.AbstractBlock) (map[economymodel.VerifierType]*big.Int, error) {
	if model.DIPErr != nil {
		return nil, EconomyErr
	}
	rewardMap := make(map[economymodel.VerifierType]*big.Int, 3)
	rewardMap[economymodel.MasterVerifier] = big.NewInt(300)
	rewardMap[economymodel.CommitVerifier] = big.NewInt(100)
	rewardMap[economymodel.NotCommitVerifier] = big.NewInt(0)
...
func (model fakeEconomyModel) GetFoundation() economymodel.Foundation {
	return nil
}

func (model fakeEconomyModel) CheckAddressType(address common.Address) economymodel.EconomyModelAddress {
	panic("implement me")
}

func (model fakeEconomyModel) GetDiffVerifierAddress(preBlock, block model.AbstractBlock) (map[economymodel.VerifierType][]common.Address, error) {
	if model.addrErr != nil {
		return nil, model.addrErr
	}
	addressMap := make(map[economymodel.VerifierType][]common.Address, 1)
	addressMap[economymodel.MasterVerifier] = VerifierAddress[:2]
