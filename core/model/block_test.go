...
func Test_RlpHash(t *testing.T) {
	...
	data := make([]byte, 0)
	tx := NewTransaction(uint64(testNonce), to, value, TestGasPrice, TestGasLimit, data)
...
func creatBlockWithAllTx(n int, t *testing.T) *Block {
	...
	tempTx := NewTransaction(uint64(0), bobAddr, big.NewInt(1000), TestGasPrice, TestGasLimit, []byte{})
