...
func TestCreateRawLockTx(t *testing.T) {
	...
	tx := CreateRawLockTx(1, hashLock, big.NewInt(34564), big.NewInt(10000), big.NewInt(1), TxGas, aliceAddr, bobAddr)
	tx.SignTx(key1, fs)
...
func TestCreateRawRefundTx(t *testing.T) {
	key1, _ := CreateKey()
	fs := NewSigner(big.NewInt(1))
	tx := CreateRawRefundTx(1, big.NewInt(10000), big.NewInt(1), TxGas, aliceAddr, bobAddr)
	tx.SignTx(key1, fs)
...
func TestCreateRawClaimTx(t *testing.T) {
	...
	tx := CreateRawClaimTx(1, hashKey, big.NewInt(10000), big.NewInt(1), TxGas, aliceAddr, bobAddr)
	tx.SignTx(key2, fs)
