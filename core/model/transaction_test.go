...
func TestNewTransaction(t *testing.T) {
	result := NewTransaction(1, common.HexToAddress("123"), big.NewInt(100), TestGasPrice, TestGasLimit, []byte{123})
...
func TestTransactionPriceNonceSort(t *testing.T) {
	...
	for start, key := range keys {
		...
		for i := 0; i < 2; i++ {
			tx := NewTransaction(uint64(start+i), common.Address{}, big.NewInt(100), TestGasPrice, TestGasLimit, nil)
...
func TestTransaction_GetReceipt(t *testing.T) {
	...
	receiptPara := ReceiptPara{Logs: []*Log{}}
	...
	assert.Equal(t, ReceiptStatusSuccessful, receipt.Status)
}
