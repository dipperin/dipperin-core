...
func TestNewRegisterTransaction(t *testing.T) {
	k1, _ := CreateKey()
	trans := NewRegisterTransaction(1, big.NewInt(50), TestGasPrice, TestGasLimit)
...
func TestNewUnStakeTransaction(t *testing.T) {
	trans := NewUnStakeTransaction(3, TestGasPrice, TestGasLimit)
	assert.EqualValues(t, trans.GetType(), common.AddressTypeUnStake)
}

func TestNewCancelTransaction(t *testing.T) {
	trans := NewCancelTransaction(3, TestGasPrice, TestGasLimit)
...
func TestNewEvidenceTransaction(t *testing.T) {
	...
	trans := NewEvidenceTransaction(3, TestGasPrice, TestGasLimit, &target, voteA, voteA)
	assert.EqualValues(t, trans.GetType(), common.AddressTypeEvidence)
}

func TestNewUnNormalTransaction(t *testing.T) {
	trans := NewUnNormalTransaction(3, big.NewInt(5), TestGasPrice, TestGasLimit)
