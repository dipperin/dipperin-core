...
func TestCalculator_GetElectPriority_Multi(t *testing.T) {
	...
	fmt.Println("TestCalculator_GetElectPriority_Multi stakeCount info", "stakeCount", stakeCount.Int64())
	...
	for j := uint(0); j < round; j++ {
		...
		for i := 0; i < len(accounts); i++ {
			...
			if err != nil {
				fmt.Println("TestCalculator_GetElectPriority_Multi calReputation", "reputation", reputation)
				panic("calReputation err")
			}
			priority, err := CalPriority(luck, reputation)
			if err != nil {
				fmt.Println("TestCalculator_GetElectPriority_Multi  priority err", "err ", err)
...
func PrintAccountInfo(tAccounts []*TestAccount) {
	for i := range tAccounts {
		fmt.Println("PrintAccountInfo", "account", tAccounts[i])
	}
}
...
func PrintAccountElectResult(accountsMap map[uint][]*TestAccount, round uint, stakeCount *big.Int, topVerifierNum uint) {
	...
	for key, value := range resultMap {
		fmt.Println("PrintAccountElectResult", "key", key, "count", value.count, "count percentage", float64(value.count)/float64(round)/float64(topVerifierNum), "stake", value.stake, "stake percentage", float64(value.stake.Int64())/float64(stakeCount.Int64()), "nonce", value.nonce, "performance", value.performance)
		if value.performance > 90 {
			performanceCount++
		}
	}
	fmt.Println("PrintAccountElectResult", "performanceCount > 90  ", performanceCount)
