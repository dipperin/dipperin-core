...
func TestNewBlockBroadcaster_getPeersWithoutBlock(t *testing.T) {
	...
	fakeBlock := createTestBlock(diff, 1)
...
func TestNewBlockBroadcaster_broadcastBlock(t *testing.T) {
	...
	mockPeer.EXPECT().ID().Return("1").AnyTimes()
	mockPeer.EXPECT().NodeName().Return("Test").AnyTimes()
	mockPeer.EXPECT().NodeType().Return(uint64(chainconfig.NodeTypeOfNormal)).AnyTimes()
...
func TestNewBlockBroadcaster_BroadcastBlock(t *testing.T) {
	...
	mockPeer1.EXPECT().NodeType().Return(uint64(chainconfig.NodeTypeOfNormal)).AnyTimes()
	...
	mockPeer2.EXPECT().NodeType().Return(uint64(chainconfig.NodeTypeOfVerifier)).AnyTimes()
