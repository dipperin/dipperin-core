...
func Test_send2MinerMaster(t *testing.T) {
	...
	mockNodeConf.EXPECT().GetNodeType().Return(chainconfig.NodeTypeOfMineMaster)
	...
	mockPeer.EXPECT().NodeName().Return("test").AnyTimes()
	mockPeer.EXPECT().SendMsg(gomock.Any(), gomock.Any()).Return(nil).AnyTimes()
	mockPeer.EXPECT().NodeType().Return(uint64(chainconfig.NodeTypeOfVerifier))
	...
	mockPeer2.EXPECT().NodeType().Return(uint64(chainconfig.NodeTypeOfMineMaster))
