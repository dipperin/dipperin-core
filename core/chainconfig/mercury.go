...
import (
	...
	"github.com/dipperin/dipperin-core/core/chainconfig/envconf"
...
func mercuryKBoots() []*enode.Node {
	...
	pkByte, _ := hexutil.Decode(envconf.MercuryBootNodePublicKey)
...
func NewMercuryVBoots() []*enode.Node {
	...
	for i := 0; i < config.VerifierBootNodeNumber; i++ {
		pkByte, _ := hexutil.Decode(envconf.MercuryVerBootPublicKey[i])
		cs_crypto.ToECDSAPub(pkByte)
