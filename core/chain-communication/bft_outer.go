...
func (broadcaster *BftOuter) onVerifiedResultBlockHash(msg p2p.Msg, p PmAbstractPeer) error {
	...
	if err := msg.Decode(&resultHash); err != nil {
		log.Error("con't decode block hash msg", "msg code", msg.Code, "err", err)
		return err
	}
	...
	log.Info("receive new verified block hash msg", "node", p.NodeName(), "height", resultHash.BlockNumber)
	...
	if broadcaster.Chain.GetBlockByNumber(resultHash.BlockNumber) != nil {
		log.Info("already have result block, no fetch block", "height", resultHash.BlockNumber)
		return nil
	}
	...
	go func() {
		...
		log.Debug("block fetcher notify", "number", resultHash.BlockNumber)
...
func (broadcaster *BftOuter) onGetVerifiedResult(msg p2p.Msg, p PmAbstractPeer) error {
	...
	if block == nil || len(seen) == 0 {
		log.Error("can't load block for remote", "seen len", len(seen), "block_is_nil?", block == nil)
		return nil
	}
...
func (broadcaster *BftOuter) newBlockReceiver(peer PmAbstractPeer) *blockReceiver {
	...
	go func() {
		...
		if err := receiver.broadcast(getPeer); err != nil {
			log.Error("broadcast verified block result hash error", "err", err, "peer name", peer.NodeName())
			return
		}
