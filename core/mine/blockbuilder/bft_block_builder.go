...
func (builder *BftBlockBuilder) commitTransaction(conf *stateprocessor.TxProcessConfig, state *chain.BlockProcessor) error {
	snap := state.Snapshot()
...
func (builder *BftBlockBuilder) commitTransactions(txs *model.TransactionsByFeeAndNonce, state *chain.BlockProcessor, header *model.Header, vers []model.AbstractVerification) (txBuf []model.AbstractTransaction, receipts model.Receipts) {
	var invalidList []*model.Transaction
	log.DLogger.Info("BftBlockBuilder#commitTransactions  start ~~~~~++")
	metricTimer := gmetrics.NewTimer(gmetrics.CommitTxsDuration)
	if metricTimer != nil {
...
func (builder *BftBlockBuilder) BuildWaitPackBlock(coinbaseAddr common.Address, gasFloor, gasCeil uint64) model.AbstractBlock {
	...
	timer := gmetrics.NewTimer(gmetrics.PackageBlockDuration)
	if timer != nil {
...
func CalcGasLimit(parent *model.Block, gasFloor, gasCeil uint64) uint64 {
	...
	contrib := (parent.GasUsed() + parent.GasUsed()/2) / model.GasLimitBoundDivisor

	// decay = parentGasLimit / 1024 -1
	decay := parent.GasLimit()/model.GasLimitBoundDivisor - 1
	...
	*/
	...
	if limit < model.MinGasLimit {
		limit = model.MinGasLimit
	}
