...
func TestTxList_Add(t *testing.T) {
	...
	tx1 := model.NewTransaction(1, bob, big.NewInt(1), model.TestGasPrice, model.TestGasLimit, []byte{})
	tx1.SignTx(keyAlice, ms)

	tx2 := model.NewTransaction(2, bob, big.NewInt(2), big.NewInt(2), model.TestGasLimit, []byte{})
	tx2.SignTx(keyAlice, ms)

	tx3 := model.NewTransaction(2, bob, big.NewInt(3), big.NewInt(3), model.TestGasLimit, []byte{})
	tx3.SignTx(keyAlice, ms)

	tx4 := model.NewTransaction(1, bob, big.NewInt(3), big.NewInt(0), model.TestGasLimit, []byte{})
...
func TestTxList_Filter(t *testing.T) {
	...
	removed, invalids := l.Filter(big.NewInt(0).Mul(big.NewInt(5), big.NewInt(int64(model.TestGasLimit))))
