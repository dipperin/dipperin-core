...
type WalletInfo struct {
	Accounts   []accountsbase.Account                         // used account included in the wallet
	Paths      map[common.Address]accountsbase.DerivationPath // derived path of the account in the wallet
...
func NewHdWalletInfo() (info *WalletInfo) {
	...
	walletInfo := WalletInfo{
		Accounts:         make([]accountsbase.Account, 0),
		Paths:            make(map[common.Address]accountsbase.DerivationPath, 0),
...
type WalletInfoJson struct {
	Accounts         []accountsbase.Account                 `json:"accounts"`
	Paths            map[string]accountsbase.DerivationPath `json:"paths"`
...
func NewHdWalletInfoJson() (jsonInfo *WalletInfoJson) {
	w := &WalletInfoJson{
		Accounts:         make([]accountsbase.Account, 0),
		Paths:            make(map[string]accountsbase.DerivationPath, 0),
...
func (w *WalletInfo) GenerateKeyFromSeedAndPath(derivedPath string, index uint32) (derivedKey *ExtendedKey, Path accountsbase.DerivationPath, err error) {
	w.lock.Lock()
	defer w.lock.Unlock()
	...
	if err != nil {
		return nil, accountsbase.DerivationPath{}, err
	}

	//Get the first account information on the default derived path based on the master key
	Path, err = accountsbase.ParseDerivationPath(derivedPath)
	if err != nil {
...
func (w *WalletInfo) paddingUsedAccount(GetAddressRelatedInfo accountsbase.AddressInfoReader) (err error) {
...
func (w *WalletInfo) PaddingAddressNonce(GetAddressRelatedInfo accountsbase.AddressInfoReader) (err error) {
	w.lock.Lock()
	defer w.lock.Unlock()
