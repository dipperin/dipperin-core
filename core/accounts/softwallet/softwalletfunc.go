...
func GetAccountFromExtendedKey(keyData *ExtendedKey) (account accountsbase.Account, err error) {
...
func CheckDerivedPathValid(path accountsbase.DerivationPath) (bool, error) {
	defaultPath, err := accountsbase.ParseDerivationPath(DefaultDerivedPath)
	if err != nil {
		return false, err
	}
...
func CheckPassword(password string) (err error) {
	...
	if reg.MatchString(password) && len(strs) <= 0 && len(blankStr) <= 0 {
		if len(password) >= accountsbase.PasswordMin && len(password) <= accountsbase.PassWordMax {
			return nil
		}
	}
