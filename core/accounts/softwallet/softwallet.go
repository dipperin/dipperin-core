...
func (w *SoftWallet) encryptWalletAndWriteFile(operation int) (err error) {
	...
	if operation == CloseWallet {
		if exist == false {
			...
			return accountsbase.ErrWalletFileNotExist
		}
	} else {
		if exist == true {
			//file already exists when creating a new wallet
			return accountsbase.ErrWalletFileExist
		} else {
...
func (w *SoftWallet) decryptWallet(password string) (passwordValid bool, walletPlain []byte, keyData EncryptKey, err error) {
	...
	if err1 != nil {
		...
		err = accountsbase.ErrWalletPasswordNotValid
		return
	}
...
func (w *SoftWallet) GetWalletIdentifier() (accountsbase.WalletIdentifier, error) {
	...
	if w.status != accountsbase.Opened {
		return accountsbase.WalletIdentifier{}, accountsbase.ErrWalletNotOpen
	}
...
func (w *SoftWallet) Close() error {
	...
	if w.status != accountsbase.Opened {
		return accountsbase.ErrWalletNotOpen
	}
...
func (w *SoftWallet) Accounts() ([]accountsbase.Account, error) {
	...
	defer w.mu.Unlock()
	if w.status != accountsbase.Opened {
		return []accountsbase.Account{}, accountsbase.ErrWalletNotOpen
	}
...
func (w *SoftWallet) Contains(account accountsbase.Account) (bool, error) {
	...
	defer w.mu.Unlock()
	if w.status != accountsbase.Opened {
		return false, accountsbase.ErrWalletNotOpen
	}
...
func (w *SoftWallet) Derive(path accountsbase.DerivationPath, save bool) (accountsbase.Account, error) {
	...
	if err != nil || !isValid {
		return accountsbase.Account{}, accountsbase.ErrInvalidDerivedPath
	}
...
func (w *SoftWallet) SignHash(account accountsbase.Account, hash []byte) ([]byte, error) {
	...
	if w.status != accountsbase.Opened {
		return []byte{}, accountsbase.ErrWalletNotOpen
	}
...
func (w *SoftWallet) SignTx(account accountsbase.Account, tx *model.Transaction, chainID *big.Int) (*model.Transaction, error) {
	...
	defer w.mu.RUnlock()
	if w.status != accountsbase.Opened {
		return nil, accountsbase.ErrWalletNotOpen
	}
...
func (w *SoftWallet) Evaluate(account accountsbase.Account, seed []byte) (index [32]byte, proof []byte, err error) {
	...
	defer w.mu.RUnlock()
	if w.status != accountsbase.Opened {
		return [32]byte{}, []byte{}, accountsbase.ErrWalletNotOpen
	}
