...
type SoftWallet struct {
	...
	Identifier accountsbase.WalletIdentifier //Wallet identifier
}
...
func NewSoftWallet() (*SoftWallet, error) {
	...
	wallet := &SoftWallet{
		...
		status:     accountsbase.Closed,
		mu:         sync.RWMutex{},
		Identifier: accountsbase.WalletIdentifier{WalletType: accountsbase.SoftWallet, Path: "", WalletName: ""},
	}
...
func (w *SoftWallet) paddingWalletInfo(mnemonic, password, passPhrase string, kdfPara *KDFParameter) (err error) {
	...
	var tmpPath accountsbase.DerivationPath
	...
	//log.DLogger.Debug("the given is: ","given",*given)
...
func (w *SoftWallet) encryptWalletAndWriteFile(operation int) (err error) {
	...
	if operation == CloseWallet {
		if exist == false {
			...
			return gerror.ErrWalletFileNotExist
		}
	} else {
		if exist == true {
			//file already exists when creating a new wallet
			return gerror.ErrWalletFileExist
		} else {
...
func (w *SoftWallet) decryptWallet(password string) (passwordValid bool, walletPlain []byte, keyData EncryptKey, err error) {
	...
	if err != nil {
		log.DLogger.Info("the expect is:", zap.Error(err))
		return
	}
	...
	if err1 != nil {
		...
		err = gerror.ErrWalletPasswordNotValid
		return
	}
...
func (w *SoftWallet) GetWalletIdentifier() (accountsbase.WalletIdentifier, error) {
	w.mu.Lock()
	defer w.mu.Unlock()

	if w.status != accountsbase.Opened {
		return accountsbase.WalletIdentifier{}, gerror.ErrWalletNotOpen
	}
...
func (w *SoftWallet) Establish(path, name, password, passPhrase string) (string, error) {
	...
	//expect = CheckPassword(passPhrase)
	//if expect != nil {
	//	return "", expect
	//}
	w.Identifier.WalletName = name
	w.Identifier.Path = path
	w.Identifier.WalletType = accountsbase.SoftWallet
...
func (w *SoftWallet) RestoreWallet(path, name, password, passPhrase, mnemonic string, GetAddressRelatedInfo accountsbase.AddressInfoReader) (err error) {
...
func (w *SoftWallet) Close() error {
	...
	if w.status != accountsbase.Opened {
		return gerror.ErrWalletNotOpen
	}
...
func (w *SoftWallet) Accounts() ([]accountsbase.Account, error) {
	w.mu.Lock()
	defer w.mu.Unlock()
	if w.status != accountsbase.Opened {
		return []accountsbase.Account{}, gerror.ErrWalletNotOpen
	}
	return w.walletInfo.Accounts, nil
}

//determine if the soft wallet contains an account
func (w *SoftWallet) Contains(account accountsbase.Account) (bool, error) {
	w.mu.Lock()
	defer w.mu.Unlock()
	if w.status != accountsbase.Opened {
		return false, gerror.ErrWalletNotOpen
	}
...
func (w *SoftWallet) Derive(path accountsbase.DerivationPath, save bool) (accountsbase.Account, error) {
	w.mu.Lock()
	defer w.mu.Unlock()
	...
	if path.String() == "m" {
		tmpPath, err = accountsbase.ParseDerivationPath(DefaultDerivedPath)
		if err != nil {
	...
	if err != nil || !isValid {
		return accountsbase.Account{}, gerror.ErrInvalidDerivedPath
	}
...
func (w *SoftWallet) SelfDerive(base accountsbase.DerivationPath) error {
	return nil
}

//Sign the hash value with its corresponding private key based on the incoming account
func (w *SoftWallet) SignHash(account accountsbase.Account, hash []byte) ([]byte, error) {
	w.mu.RLock()
	defer w.mu.RUnlock()

	if w.status != accountsbase.Opened {
		return []byte{}, gerror.ErrWalletNotOpen
	}
...
func (w *SoftWallet) SignTx(account accountsbase.Account, tx *model.Transaction, chainID *big.Int) (*model.Transaction, error) {
	// Transaction signature operation with the private key based on the account
	w.mu.RLock()
	defer w.mu.RUnlock()
	if w.status != accountsbase.Opened {
		return nil, gerror.ErrWalletNotOpen
	}
...
func (w *SoftWallet) Evaluate(account accountsbase.Account, seed []byte) (index [32]byte, proof []byte, err error) {
	w.mu.RLock()
	defer w.mu.RUnlock()
	if w.status != accountsbase.Opened {
		return [32]byte{}, []byte{}, gerror.ErrWalletNotOpen
	}
...
func (w *SoftWallet) GetPKFromAddress(account accountsbase.Account) (*ecdsa.PublicKey, error) {
...
func (w *SoftWallet) PaddingAddressNonce(GetAddressRelatedInfo accountsbase.AddressInfoReader) (err error) {
