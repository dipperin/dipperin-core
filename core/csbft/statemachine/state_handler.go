...
func (h *StateHandler) OnNewHeight(height uint64) {
	...
	if h.metricTimer == nil {
		h.metricTimer = gmetrics.NewTimer(gmetrics.PBFTValidBlockDuration)
	}
	round := uint64(0)
	...
	if height > 1 {
		...
		if len(seenCommit) == 0 {
			log.DLogger.Error(gerror.ErrCannotLoadSeenCommit.Error(), zap.Uint64("chainHeight", chainHeight))
			return
		}
...
func (h *StateHandler) finalBlock(block model.AbstractBlock, commits []model.AbstractVerification) {
	...
	if err != nil {
		...
		if err.Error() != gerror.ErrAlreadyHaveThisBlock.Error() {
			return
		}
	}
...
func (h *StateHandler) addTimeoutCount(label string) {
	gmetrics.Add(gmetrics.BftTimeoutCount, label, 1)
}
