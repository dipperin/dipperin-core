...
func (bft *CsBft) Start() error {
	pbft_log.Log.Info("start CsBft", "cur height", bft.ChainReader.CurrentBlock().Number())
	if !bft.canStart() {
		pbft_log.Log.Info("isn't cur verifier, can't start CsBft", "err", g_error.ErrIsNotCurVerifierCannotStartBft)
		return nil
	}
	...
	pbft_log.Log.Debug("start git", "is running", bft.stateHandler.IsRunning(), "err", err)
	err = bft.blockPool.Start()
	pbft_log.Log.Debug("start pool", "is running", bft.blockPool.IsRunning(), "err", err)
	err = bft.fetcher.Start()
	pbft_log.Log.Debug("start fetcher", "is running", bft.fetcher.IsRunning(), "err", err)

	return nil
}

func (bft *CsBft) Stop() {
	pbft_log.Log.Info("stop CsBft", "cur height", bft.ChainReader.CurrentBlock().Number())

	bft.stateHandler.Stop()
	if err := bft.stateHandler.Reset(); err != nil {
		log.Warn("reset state handler failed", "err", err)
	}
	pbft_log.Log.Debug("Stop state handler", "state handler is running", bft.blockPool.IsRunning())
	bft.blockPool.Stop()
	pbft_log.Log.Debug("Stop pool", "pool is running", bft.blockPool.IsRunning())
	bft.fetcher.Stop()
	pbft_log.Log.Debug("Stop fetcher", "fetcher is running", bft.blockPool.IsRunning())
	bft.fetcher.Reset()
}

func (bft *CsBft) OnNewWaitVerifyBlock(block model.AbstractBlock, id string) {
	//pbft_log.Log.Debug("cs onNewWatVerifyBlock")
	//check the node is or isn't current verifier node
	if !bft.stateHandler.IsRunning() || !bft.blockPool.IsRunning() {
		pbft_log.Log.Debug("cs onNewWatVerifyBlock, bft not running")
		return
	}
	pbft_log.Log.Info("cs bft OnNewWaitVerifyBlock", "block num", block.Number())

	// todo check block valid here?
	if err := bft.blockPool.AddBlock(block); err != nil {
		pbft_log.Log.Info("pool add block failed", "err", err)
		return
	}
...
func (bft *CsBft) broadcastFetchBlockMsg(blockHash common.Hash) {
	...
	pbft_log.Log.Info("broadcast sync block msg", "hash", blockHash.Hex())
...
func (bft *CsBft) ChangePrimary(primary string) {
	pbft_log.Log.Debug("Change Primary Called")
	pbft_log.Log.Debug("Current num", "num", bft.ChainReader.CurrentBlock().Number())
	if bft.canStart() {
		pbft_log.Log.Debug("Start state handler")
		bft.Start()
		bft.stateHandler.NewHeight(bft.ChainReader.CurrentBlock().Number() + 1)
		return
	}
	pbft_log.Log.Debug("Stop state handler")
	bft.Stop()
}
...
func (bft *CsBft) isCurrentVerifier() bool {
	...
	pbft_log.Log.Info("CsBft isCurrentVerifier", "cur vs", vs, "cur account", curAccount)
	for _, v := range vs {
...
func (bft *CsBft) OnNewP2PMsg(msg p2p.Msg, p chain_communication.PmAbstractPeer) error {
	if !bft.stateHandler.IsRunning() {
		pbft_log.Log.Warn("[Node-OnNewMsg]receive bft msg, but state handler not started")
		return nil
	}
	...
	case model2.TypeOfNewRoundMsg:
		...
		pbft_log.Log.Info("[Node-OnNewMsg]receive new round msg", "node", p.NodeName(), "height", m.Height, "round", m.Round)
	...
	case model2.TypeOfProposalMsg:
		...
		pbft_log.Log.Info("[Node-OnNewMsg]receive proposal msg", "node", p.NodeName(), "height", m.Height, "round", m.Round, "block", m.BlockID.Hex())
	...
	case model2.TypeOfPreVoteMsg:
		...
		pbft_log.Log.Info("[Node-OnNewMsg]receive prevote msg", "node", p.NodeName(), "height", m.Height, "round", m.Round, "block", m.BlockID.Hex())
	...
	case model2.TypeOfVoteMsg:
		...
		pbft_log.Log.Info("[Node-OnNewMsg]receive vote msg", "node", p.NodeName(), "height", m.Height, "round", m.Round, "block", m.BlockID.Hex())
		bft.stateHandler.Vote(&m)

	case model2.TypeOfFetchBlockReqMsg:
		//fmt.Println("receive fetch block msg")
		pbft_log.Log.Info("[Node-OnNewMsg]receive fetch block msg", "from", p.NodeName())
		...
		pbft_log.Log.Info("[Node-OnNewMsg] fetch result", "to", p.NodeName(), "block_is_nil", b == nil)
		...
		if err := p.SendMsg(uint64(model2.TypeOfFetchBlockRespMsg), &components.FetchBlockRespMsg{
			...
			pbft_log.Log.Warn("[Node-OnNewMsg] send fetch block to client failed", "err", err)
		}
		pbft_log.Log.Info("[Node-OnNewMsg] send fetch result 2")

	case model2.TypeOfFetchBlockRespMsg:
		var m model2.FetchBlockRespDecodeMsg
		if err := msg.Decode(&m); err != nil {
			pbft_log.Log.Debug("[Node-OnNewMsg] Decode Error, FetchBlockRespMsg", "err", err)
			return err
		}
		pbft_log.Log.Info("[Node-OnNewMsg] receive fetch block resp", "node", p.NodeName())
	...
	case model2.TypeOfSyncBlockMsg:
		pbft_log.Log.Info("[Node-OnNewMsg] receive sync block", "node", p.NodeName())
		var m common.Hash
		if err := msg.Decode(&m); err != nil {
			return err
		}
	...
	case model2.TypeOfReqNewRoundMsg:
		...
		if err := msg.Decode(&m); err != nil {
			pbft_log.Log.Error("decode req new round msg error", "err", err)
			return err
		}
		pbft_log.Log.Info("[Node-OnNewMsg] receive req new round", "node", p.NodeName(), "height", m.Height, "round", m.Round)

		msg := bft.stateHandler.GetRoundMsg(m.Height, m.Round)
		pbft_log.Log.Debug("[Node-OnNewMsg] response", "msg == nil", msg == nil)
		if msg != nil {
			pbft_log.Log.Info("[Node-OnNewMsg]  response round request", "to", p.NodeName(), "height", m.Height, "round", m.Round)
			if err := p.SendMsg(uint64(model2.TypeOfNewRoundMsg), msg); err != nil {
				pbft_log.Log.Error("response round request error", "err", err)
			}
		}
	default:
...
func (bft *CsBft) onSyncBlockMsg(from common.Address, h common.Hash) {
	...
	if from.IsEmpty() {
		pbft_log.Log.Warn("from is empty, do nothing for sync msg")
		return
	}
	if h.IsEmpty() {
		pbft_log.Log.Warn("block hash is empty, do nothing for sync msg")
		return
	}

	if !bft.blockPool.IsEmpty() {
		//pbft_log.Log.Warn("pool not empty, ignore sync block msg")
		return
	}
	...
	if b != nil {
		pbft_log.Log.Info("onSyncBlockMsg already have this block")
		return
	}
	...
	if b != nil {
		if err := bft.blockPool.AddBlock(b); err != nil {
			pbft_log.Log.Warn("fetcher add block failed", "err", err)
		}
		return
	}
	pbft_log.Log.Info("fetch block failed")
}
